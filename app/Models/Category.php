...
//        return $builder->where('id', 2);
        //cat filter with subscription of members start
        $authUser = auth('sanctum')->user();
        if($authUser)
        {
            $user__Type = $authUser->usr_type;
            $user__Id = $authUser->id;
            $ptype = "App\Models\User";
    ...
    //            return $builder->where('id', 2);
                ...
                $activeSubs = DB::table('payments')->select('package_id')->where('payable_id',$user__Id)->where('payable_type',$ptype)->where('active','1')->where('period_start', '<=', $currentDate)->where('period_end', '>=', $currentDate)->orderBy('id', 'desc')->get();
                $activeSubID=$activeSubs[0]->package_id;
                if($activeSubID>0)
                {

                    $activeComb = DB::table('packages')->select('comb_id')->where('id',$activeSubID)->get();
                    $activeCombID=$activeComb[0]->comb_id;

                    $activeSubCats = DB::table('sub_cat_combinations')->select('sub_cat_ids')->where('id',$activeCombID)->get();
                    $activeSubCatIDs=$activeSubCats[0]->sub_cat_ids;

                    $activeSubCatIDsArr=(explode(",",$activeSubCatIDs));

                    if(count($activeSubCatIDsArr)>1)
                    {
                        return $builder->where('parent_id', $parentId)->whereIn('id', $activeSubCatIDsArr);
                    }
                    elseif(count($activeSubCatIDsArr)==1)
                    {

                        $testcombcat = $activeSubCatIDsArr[0];
                        $activeCat = DB::table('categories')->select('parent_id')->where('id',$testcombcat)->get();
                        $activeCatID=$activeCat[0]->parent_id;
                        if($activeCatID>0)
                        {
                            return $builder->where('parent_id', $parentId)->where('id', $testcombcat);
                        }
                        else
                        {
                            if($parentId==$testcombcat)
                            {
                                return $builder->where('parent_id', $testcombcat);
                            }
                            else
                            {
                                return $builder->where('parent_id', '9999999');
                            }
                        }
                    }
                    else
        ...
        //cat filter with subscription of members end
