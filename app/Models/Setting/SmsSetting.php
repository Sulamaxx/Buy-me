...
class SmsSetting
{
	public static function getValues($value, $disk)
	{
		if (empty($value)) {
			...
			$value['notify_senderid'] = env('NOTIFY_SENDERID', '');
			$value['notify_key'] = env('NOTIFY_KEY', '');
			$value['notify_token'] = env('NOTIFY_TOKEN', '');
	...
	public static function getFields($diskName)
	{
		$fields = [
			...
			[
				'name'              => 'driver_notify_title',
				'type'              => 'custom_html',
				'value'             => 'Notify LK Title',
				'wrapperAttributes' => [
					'class' => 'col-md-12 notify',
				],
			],
			[
				'name'              => 'driver_notify_info',
				'type'              => 'custom_html',
				'value'             => 'Info',
				'wrapperAttributes' => [
					'class' => 'col-md-12 notify',
...
function getDriverFields(driverEl) {
	...
	hideEl(document.querySelectorAll(".vonage, .twilio,.notify"));
	
	if (driverElValue === "vonage") {
		showEl(document.querySelectorAll(".vonage"));
	}

	if (driverElValue === "notify") {
		showEl(document.querySelectorAll(".notify"));
	}
