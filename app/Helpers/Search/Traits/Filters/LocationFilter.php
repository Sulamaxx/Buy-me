...
trait LocationFilter
{
    ...
    protected function applyLocationFilter(): void
    {
        ...
        // Get selected location IDs from the request (e.g., 'l[]' from multi-select)
        $selectedLocations = request()->input('l', []);

        if (!empty($selectedLocations)) {
            // Filter by multiple city IDs
            $this->posts->whereIn('city_id', (array)$selectedLocations);
        } else {
            // Fallback to existing logic if no locations are selected
        ...
        // Remove distance from request if not needed
