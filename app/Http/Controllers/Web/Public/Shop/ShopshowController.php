...
namespace App\Http\Controllers\Web\Public\shop;
...
class ShopshowController extends FrontController
{
	...
	public function index($shopId)
	{
		...
		if (config('settings.seo.listing_hashed_id_seo_redirection')) {
			if (config('settings.seo.listing_hashed_id_enabled') && !isHashedId($shopId) && is_numeric($shopId)) {
				// Don't lose important notification, so we need to persist your flash data for the request (the redirect request)
				request()->session()->reflash();
				
				$uri = UrlGen::postPathBasic(hashId($shopId), $slug);
//				dd($uri);
			...
			if (!config('settings.seo.listing_hashed_id_enabled') && isHashedId($shopId) && !is_numeric($shopId)) {
				...
				$uri = UrlGen::postPathBasic(hashId($shopId, true), $slug);
//				dd($uri);
				return redirect()->to($uri, 301)->withHeaders(config('larapen.core.noCacheHeaders'));
			}
		}
		...
		// Decode Hashed ID
		$shopId = hashId($shopId, true) ?? $shopId;
		...
		$headers = session()->has('postIsVisited') ? ['X-VISITED-BY-SAME-SESSION' => $shopId] : [];
...
//		dd($data);
		...
		session()->put('postIsVisited', $shopId);
	 ...
	 * @return array|null
	 */
	public function similarPosts($shopId): ?array
	{
		$post = null;
		$posts = [];
		$totalPosts = 0;
