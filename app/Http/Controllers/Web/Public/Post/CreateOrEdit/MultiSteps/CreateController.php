...
class CreateController extends FrontController
{
    ...
    public function notifypostphones($phoneNumber)
    {
        
        $user = auth('sanctum')->user();
        $userID = $user->id;
        $deleted = DB::table('post_phone_verify')->where('user_id', '=', $userID)->delete();
        $newnotification = new NotifyChannelforpost;
        $phone_token = mt_rand(100000, 999999);

        $phoneNumber = "94".$phoneNumber;
        $insrtID = DB::table('post_phone_verify')->insertGetId([ 
    'user_id' => $userID,
    'phone' => $phoneNumber,
    'phone_token' => $phone_token,
    'verified' => 0
]);
        $newnotification->send($phone_token,$phoneNumber);
        return $phoneNumber."Test Success.".$phone_token. " - ".$insrtID;
    }
    
    public function veryfypostphones($phoneNumber,$phtoken)
    {
        $user = auth('sanctum')->user();
        $userID = $user->id;
        $phoneNumber = "94".$phoneNumber;
        $tokenfromdb = DB::table('post_phone_verify')->where('phone', $phoneNumber)->where('user_id', $userID)->value('phone_token');
 
        if($tokenfromdb==$phtoken)
             {
                return "verified";
                $deleted = DB::table('post_phone_verify')->where('user_id', '=', $userID)->delete();
             }
