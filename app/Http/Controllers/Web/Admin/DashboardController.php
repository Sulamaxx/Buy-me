...
class DashboardController extends PanelController
{
	...
	public function dashboard()
	{
        ...
        $this->data['revivedPosts'] = Post::with([
			'payment',
			'payment.package',
			'user',
			'country',
		])->join('users', 'posts.reviewed_user_id', '=', 'users.id')->where('reviewed_user_id','>', 0)->select('posts.*', 'users.name')->take($latestEntriesLimit)->orderByDesc('posts.created_at')->get();
        
        $this->data['updatedPosts'] = Post::with([
			'payment',
			'payment.package',
			'user',
			'country',
		])->join('users', 'posts.updated_user_id', '=', 'users.id')->where('updated_user_id','>', 0)->select('posts.*', 'users.name')->take($latestEntriesLimit)->orderByDesc('posts.created_at')->get();
		
        // Get Revived Ads
//         $this->data['revivedPosts'] = DB::table('posts')
//            ->where('reviewed_user_id','>', 0)
//            ->join('users', 'posts.reviewed_user_id', '=', 'users.id')
