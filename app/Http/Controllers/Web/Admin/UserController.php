...
class UserController extends PanelController
{
	...
	public function setup()
	{
		...
		*/
        ...
        $this->xPanel->addField([
			'name'              => 'member_shop',
			'label'             => trans('Has a Shop'),
			'type'              => 'checkbox_switch',
			'wrapperAttributes' => [
				'class' => 'col-md-6',
			],
		]);
        $usrtypes = DB::table('usr_types')->select('*')->get();
        $ShopOwners[0] ="Please Select A User Type!";  
        foreach($usrtypes AS $usrtype)
        {
            $ShopOwners[$usrtype->id] =  $usrtype->usr_type;
        }
