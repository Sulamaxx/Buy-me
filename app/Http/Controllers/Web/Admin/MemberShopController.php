...
use App\Http\Requests\Admin\MemberShopRequest as StoreRequest;
use App\Http\Requests\Admin\MemberShopRequest as UpdateRequest;

class MemberShopController extends PanelController
{
	...
	public function setup()
	{
		...
		|--------------------------------------------------------------------------
		*/
		$this->xPanel->setModel('App\Models\MemberShop');
		$this->xPanel->with([
			'user:id,name',
			
		]);
		$this->xPanel->setRoute(admin_uri('member_shops'));
		$this->xPanel->setEntityNameStrings(trans('admin.shop'), trans('admin.shops'));
...
//			'function_name' => 'getOUserNameHtml',
		]);
        
		
		$this->xPanel->addColumn([
			'name'          => 'shop_banner', // Put unused field column
			'label'         => "Shop Banner",
			'type'          => 'model_function',
...
//			'function_name' => 'getCUserNameHtml',
		]);
        ...
        $this->xPanel->addColumn([
			'name'  => 'shop_expire_date',
			'label' => "Shop Expiry Date",
			'type'  => 'datetime',
		]);
...
//		$wysiwygEditorViewPath = '/views/admin/panel/fields/' . $wysiwygEditor . '.blade.php';
        ...
        $this->xPanel->addField([
			'name'       => 'shop_open_hours',
			'label'      => "Shop Open Hours",
			'type'       => 'textarea',
			'attributes' => [
				'placeholder' => "Shop  Open Hours",
				'id'          => 'shop_open_hours',
				'rows'        => 5,
			],
		]);
...
//			'name'       => 'shop_status',
//            'options'     => $Shopstatus,
//            'value'      => $entity->shop_status,
//			'label'      => "Shop Active Status",
//			'type'        => 'select2_from_array',
//			
//		]);
...
//        dd($request);
                {
                    if($request->shop_banner!=null && (!is_string($request->shop_banner) && null !==($request->shop_banner->hashName()))) {
                        ...
                        $request->shop_banner->storeAs($path,$request->user_id.'.'.$request->shop_banner->extension());
                        $filename = $path.'/'.$request->user_id.'.'.$request->shop_banner->extension();
                        $request['shop_banner'] = $filename;
                    }
//                    else
//                    {
//                        $request->shop_banner = "app/default/picture.jpg";
//                        $request['shop_banner'] = "app/default/picture.jpg";
//                    }
                }
