...
class HomeSectionController extends PanelController
{
	...
	public function reset($action)
	{
		...
		if ($action == 'reset_reorder') {
			...
			HomeSection::where('method', 'getLeftAdvertising')->update(['lft' => 18, 'rgt' => 19, 'active' => 0]);
			HomeSection::where('method', 'getRightAdvertising')->update(['lft' => 20, 'rgt' => 21, 'active' => 0]);
		...
		if ($action == 'reset_settings') {
			...
			HomeSection::where('method', 'getLeftAdvertising')->update(['value' => null, 'active' => 0]);
			HomeSection::where('method', 'getRightAdvertising')->update(['value' => null, 'active' => 0]);
	...
	public function getHomeSilederData(){
		$users = DB::table('home_slider_new')->get();
		
        return view('admin.slider.index', ['users' => $users]);
	}

	public function getHomeSilederCreate(){
        return view('admin.slider.create');
	}

	public function getHomeSilederEdit($id){
		$slider = DB::table('home_slider_new')->find($id);
        return view('admin.slider.edit', ['slider' => $slider]);
	}

	public function postHomeSilederCreate(Request $request){
		$file = $request->file('file');
		$destinationPath = public_path('uploads');
        $extension = $file->getClientOriginalExtension();
        $originalName = pathinfo($file->getClientOriginalName(), PATHINFO_FILENAME);
        $fileName = 'slider-' . uniqid() . '.' . $extension;
        $file->move($destinationPath, $fileName);
		DB::table('home_slider_new')->updateOrInsert(
			['image_name' => $fileName, 'exf_time' => $request->extime, 'isactive' => 1]
		);
		$users = DB::table('home_slider_new')->get();
	...
	public function putHomeSilederCreate(Request $request){
		if($request->file('file')){
			$file = $request->file('file');
			$destinationPath = public_path('uploads');
			$extension = $file->getClientOriginalExtension();
			$originalName = pathinfo($file->getClientOriginalName(), PATHINFO_FILENAME);
			$fileName = 'slider-update-' . uniqid() . '.' . $extension;
			$file->move($destinationPath, $fileName);
			DB::table('home_slider_new')
              ->where('id', $request->idS)
              ->update(	['image_name' => $fileName, 'exf_time' => $request->extime]);
		}else{
			DB::table('home_slider_new')
              ->where('id', $request->idS)
              ->update(	['exf_time' => $request->extime]);
		}
		
		// DB::table('home_slider')->updateOrInsert(
		// 	['image_name' => $fileName, 'exf_time' => $request->extime]
		// );
