...
trait SectionDataTrait
{
	...
	protected function getLeftAdvertising(?array $value = []): array
	{
		$cacheId = 'advertising.left';
		$leftAdvertising = cache()->remember($cacheId, $this->cacheExpiration, function () {
			return Advertising::query()
				->where('integration', 'unitSlot')
				->where('slug', 'h-left')
				->first();
		...
		return [
			'leftAdvertising' => $leftAdvertising,
		];
	}
    
	protected function getRighAdvertising(?array $value = []): array
	{
		$cacheId = 'advertising.right';
		$rightAdvertising = cache()->remember($cacheId, $this->cacheExpiration, function () {
			return Advertising::query()
				->where('integration', 'unitSlot')
				->where('slug', 'h-right')
				->first();
		...
		return [
			'rightAdvertising' => $rightAdvertising,
		];
	}
	...
	protected function getMiddleAdvertising(?array $value = []): array
	{
		$cacheId = 'advertising.middle';
		$midAdvertising = cache()->remember($cacheId, $this->cacheExpiration, function () {
			return Advertising::query()
				->where('integration', 'unitSlot')
				->where('slug', 'middle')
				->first();
