...
use App\Http\Controllers\Api\Shop\UpdateTrait;
use App\Http\Controllers\Api\Shop\StoreTrait;
use App\Http\Controllers\Api\Shop\ListTrait;
use App\Http\Controllers\Api\Shop\ShowTrait;
//use App\Http\Requests\Front\PostRequest;
use App\Http\Requests\Front\ShopRequest;
//use App\Models\Post;
use App\Models\MemberShop;
 ...
 * @group Listings
 */
class MemberShopController extends BaseController
{
...
//            dd($embed);
			return $this->showDetailedShop($id);
		}
				...
				try {
					...
					if ($smsNotificationCanBeSent) {
						if(config('settings.sms.driver') == 'twilio'){
							$smsChannel=TwilioChannel::class;
						}elseif(config('settings.sms.driver') == 'notify'){
							$smsChannel='notify';//Notify lk place
						}else{
							$smsChannel='vonage';
						}
