...
//            exit;
    ...
    public function getSimilarPosts2(): \Illuminate\Http\JsonResponse
	{
		$postId = request()->query('postId');
...
//            exit;
			...
			$distance = request()->query('distance');
			$res = $this->getSimilarPostsData2($postId, $distance);
			$posts = $res['posts'] ?? collect();
			$post = $res['post'] ?? null;
    ...
    protected function getSimilarPostsData2(?int $postId, ?int $distance = 50): array
	{
		$cacheLocaleId = '.' . config('app.locale');
...
//            echo($cacheId);
//            exit;
//            posts.similar.category.2.post.1545.en.limit.6
            $posts = $post->getSimilarByCategory2($postsLimit);
//			$posts = cache()->remember($cacheId, $this->cacheExpiration, function () use ($post, $postsLimit) {
//				return $post->getSimilarByCategory2($postsLimit);
//			});
//            print_r($posts);
//        exit();
//            echo("test 123");
