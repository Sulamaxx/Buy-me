			...
			<div class="d-md-flex">
				<div>
					<h4 class="card-title fw-bold">
						<span class="lstick d-inline-block align-middle"></span>{{ trans('admin.Reviewed Listings') }}
					</h4>
				</div>
				<div class="ms-auto">
			...
			<div class="table-responsive mt-md-3 mt-5 no-wrap">
				<table id="rvad-list" class="table v-middle mb-0">
					...
					<tr>
						...
						<th class="border-0">{{ trans('admin.revivedusr') }}</th>
                        <th class="border-0">{{ trans('admin.revDate') }}</th>
					</tr>
					</thead>
					<tbody>
					@if ($revivedPosts->count() > 0)
						@foreach($revivedPosts as $post)
...
//                                    exit();
                                ...
                                <td class="td-nowrap">{{ $post->reviewed_at }}</td>
							</tr>
						@endforeach
					@else
						<tr>
