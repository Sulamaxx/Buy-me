...
<div class="reg-sidebar-inner text-center">
	...
	@if (request()->segment(1) == 'create' || request()->segment(2) == 'create')
		...
		<div class="promo-text-box">
			...
			<p>
				<div>{{ t('do_you_have_something_text', ['url' => getUrlPageByType('ad-posting-rules')]);   }} </div>
			...
			@if (auth()->check())
				@if (auth()->user()->getAuthIdentifier() == data_get($post, 'user_id'))
					<div class="card sidebar-card panel-contact-seller">
						...
						<div class="card-content user-info">
							<div class="card-body text-center">
								<a id="btnReturnToAdPreview" href="{{ \App\Helpers\UrlGen::post($post) }}" class="btn btn-default btn-block">
									<i class="far fa-hand-point-right"></i> {{ t('Return to the listing') }}
								</a>
								<a id="btnUpdatePhotos" href="{{ url('posts/' . data_get($post, 'id') . '/photos') }}" class="btn btn-default btn-block">
								...
								@if (isset($countPackages) && isset($countPaymentMethods) && $countPackages > 0 && $countPaymentMethods > 0)
									<a id="btnSetPromoNow" href="{{ url('posts/' . data_get($post, 'id') . '/payment') }}" class="btn btn-success btn-block">
