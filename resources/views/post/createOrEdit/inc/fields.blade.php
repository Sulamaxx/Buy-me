...
@if (!empty($fields))
	@foreach($fields as $field)
		...
		@elseif ($modelFieldType == 'textarea')
			...
			<div class="row mb-3 {{ $requiredClass }}">
				...
				<div class="col-md-8">
					...
					@endphp
					<textarea style="height: 100px;" class="form-control{{ $errorClass }}"
						  id="{{ $fieldId }}"
						  name="{{ $fieldName }}"
						   
						  rows="30"{!! $fieldMaxAttr !!}
			...
			<div class="row mb-3 {{ $requiredClass }}">
				...
				<div class="col-md-8">
					...
					<input id="{{ $fieldId }}"
						   name="{{ $fieldName }}"
						   type="number"
						   placeholder="{!! data_get($field, 'help') !!}"
						   class="form-control input-md{{ $errorClass }}"
		...
		@elseif ($modelFieldType == 'label')
			
			{{-- label --}}
			<div class="row mb-8 {{ $requiredClass }}">
				<label class="col-md-9 col-form-label" for="{{ $fieldId }}">
					{{ data_get($field, 'name') }}
					@if (data_get($field, 'required') == 1)
						<sup>*</sup>
					@endif
