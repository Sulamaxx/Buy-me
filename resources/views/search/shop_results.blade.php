...
@section('content')
	<div class="main-container">
        <?php
//        dd($userShop);
//        echo($userShop[0]->shop_banner);
//        exit();
        ?>
        @includeFirst([
				config('larapen.core.customizedViewPath') . 'search.inc.sb',
				'search.inc.sb'
		...
		<?php /*?>@includeFirst([config('larapen.core.customizedViewPath') . 'search.inc.breadcrumbs', 'search.inc.breadcrumbs'])
		...
		<?php */?>
		<div class="container">
            <form id="search" name="search" action="{{ url('shop/search') }}" method="GET">
<!--            <form id="search" name="search" action="https://dev.buyme.lk/search" method="GET">-->
<!--				<div class="col-40 px-1 py-sm-1 bg-primary rounded">-->
				<div class="row gx-1 gy-1">
			
					<div class="col-xl-10 col-md-10 col-sm-12 col-12"><br>
						<input name="q" class="form-control keyword shop_search" type="text" placeholder="What are you looking for?" <?php 
                               if(isset($_GET['q']))
                               {
                                   ?>
                               ...
                               ?> >
                        <input type="hidden" name="userId" id="userId" value="{{ $userShop[0]->user_id }}" />
					</div>
					
					<div class="col-xl-2 col-md-2 col-sm-12 col-12"><br>
						<button class="btn btn-block btn-primary shop_searchbtn">
...
<!--				</div>-->
			...
			<div class="row">
				...
				<div class="col-lg-3 page-sidebar-right">
					@includeFirst([config('larapen.core.customizedViewPath') . 'search.inc.sidebarshop', 'search.inc.sidebarshop'])
				</div>
		...
		<div class="container mb-3">
			<div class="card border-light text-dark bg-light mb-3">
				<div class="card-body text-center">
					...
					@else
						<a href="{{ \App\Helpers\UrlGen::addPost() }}" class="btn btn-border btn-post btn-listing">
							{{ t('start_now') }}
						</a>
					@endif
				</div>
