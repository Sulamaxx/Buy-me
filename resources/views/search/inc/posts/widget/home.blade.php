    ...
    <div class="col-xl-12 content-box layout-section" style="background-color: transparent">
        <div class="row row-featured">
            <div class="col-xl-12 box-title" style="background-color: transparent !important">
                <div class="inner" style="background-color: transparent">
                    <h2 style="display: flex; align-items: center;padding-left:0px;">
                        <span class="title-3" style="font-weight: bold;margin-left:0px;">{{ data_get($widget, 'title', t('Latest Listings')) }}</span>
                        <div style="width: 30px; height: 2px; background-color: #52AB4A; margin-left: 5px;margin-top:13px"></div>
                        <a href="{{ data_get($widget, 'link') }}" class="sell-your-item" style="margin-left: auto;color:#666666;font-weight:bold;padding-right:0px;">
                            {{ data_get($widget, 'link_text', t('View more')) }} <i class="fas fa-chevron-right" style="color:#52AB4A"></i><i class="fas fa-chevron-right" style="color:#52AB4A"></i>
                        </a>
                    </h2>
                </div>
            </div>
            ...
            <div class="col-xl-12" style="background-color: transparent">
                <div class="row row-cols-2 row-cols-sm-3 row-cols-md-4 row-cols-lg-4 row-cols-xl-4 g-2 g-md-3" style="background-color: transparent;margin-right: -0.25vw;">
                    @foreach($posts as $key => $post)
                        {{-- Add the lazy-load-item class here --}}
                        <div class="col lazy-load-item">
                            <div class="card" style="border-radius: 7.5px;min-height: 275px">
                                ...
                                @endif
                                <div class="card-body" style="display: flex; flex-direction: column; min-height: 170px;">
    ...
    ">
        <a href="{{ \App\Helpers\UrlGen::post($post) }}" style="color: #333333;">
            ...
            {{ str(data_get($post, 'title'))->limit(50) }}
                                    ...
                                    <div>
                                        <p class="card-text" style="font-size: 0.8rem; color: #999; margin-bottom: 3px;">
                                            ...
                                            @if (data_get($post, 'city.name'))
                                                ...
                                                &nbsp;
                                                <span style="white-space: nowrap;">
                                                     <i class="fas fa-map-marker-alt"></i> {{ data_get($post, 'city.name') }}
                                                </span>
                                            @endif
                                        </p>
                                        <p class="card-text" style="font-size: 0.9rem; color: #008E00; margin-bottom: 5px;font-weight:bold">
                                            {!! data_get($post, 'price_formatted') !!}
                                        </p>
                                    </div>
                                    <p class="card-text" style="font-size: 0.7rem; color: #999; text-align: right; margin-top:auto;margin-bottom: 0;">
                                        <i class="far fa-clock"></i> {{ \Carbon\Carbon::parse(data_get($post, 'created_at'))->diffForHumans() }}
                                    </p>
                                </div>
                            </div>
                        </div>
