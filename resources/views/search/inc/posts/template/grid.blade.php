...
@if (!empty($posts) && $totalPosts > 0)
    ...
    @foreach ($posts as $key => $post)
        @if (data_get($post, 'featured') == 1)
            <div class="item-list featured-item" style="border-top: 5px solid #FDAB02; background-color: #fdab0217;">
            @else
                <div class="item-list">
        @endif
        @if (data_get($post, 'featured') == 1)
            @if (!empty(data_get($post, 'payment.package')))
                @if (data_get($post, 'payment.package.ribbon') != '')
                    <div class="ribbon-horizontal {{ data_get($post, 'payment.package.ribbon') }} aaaaaaaaaa">
                            ...
                            @if (!config('settings.list.hide_post_type') && config('settings.single.show_listing_type'))
                                @if (!empty(data_get($post, 'postType')))
                                    <span class="add-type business-posts" data-bs-toggle="tooltip"
                                        data-bs-placement="bottom" title="{{ data_get($post, 'postType.name') }}">
            ...
            <?php
                if(!empty(data_get($post, 'user.usr_type'))){
                    ...
                    if($usertypeid>1)
                    {
                        ...
                        ?>
            <div
                style="background: #848484; color: aliceblue; padding-left: 3px; width: min-content !important; border-radius:5px;align-content: center ">
                {{ $UsrType }}&nbsp;</div>
            <?php
                    }}
                    ?>
