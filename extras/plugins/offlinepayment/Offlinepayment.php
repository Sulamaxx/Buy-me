...
//				return redirect()->to(self::$uri['nextUrl']);
	...
	public static function register(Post|User $payable, array $params, array $resData = [])
	{
        ...
        if($payableType == 'App\Models\Post')
        {
            //delete previous unapproved similar payments
            DB::table('payments')->where('active',0)->where('payable_id',$payable->id)->where('package_id',data_get($params, 'package.id'))->delete();
	 }
		// Save the payment
		$paymentInfo = [
